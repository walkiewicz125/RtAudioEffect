...
#[proc_macro_derive(WireData)]
pub fn wire_data_macro_derive(input: TokenStream) -> TokenStream {
    let ast = syn::parse(input).unwrap();

    // Build the trait implementation
    wire_data::impl_wire_data_macro(&ast)
...
mod wire_data_impl {
    ...
    fn impl_struct_field(field: &Field) -> Result<SerializingCode, Error> {
    ...
    fn impl_struct(data_struct: &syn::DataStruct) -> Result<SerializingCode, Error> {
        ...
        if let syn::Fields::Named(named) = &data_struct.fields {
            for field in named.named.iter() {
                let mut field_code = impl_struct_field(field).unwrap();
                serializing_fields.append(&mut field_code.serialize);
                deserializing_fields.append(&mut field_code.deserialize);
            }
    ...
    pub fn impl_wire_data_macro(ast: &syn::DeriveInput) -> Result<TokenStream, Error> {
                    ...
                    if fields_unnamed.unnamed.len() != 1 {
                        ...
                        if let Type::Verbatim(ty) = &fields_unnamed.unnamed[0].ty {
                            ...
                            aggregate_errors(
                                ...
                                syn::Error::new(
                                    fields_unnamed.span(),
                                    "Only types that implement WireData are supported",
                                ),
                            );
                        }
