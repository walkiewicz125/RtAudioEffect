...
pub struct MagnitudeTimelineShader {
    // openGl objects ids:
...
impl MagnitudeTimelineShader {
    pub fn new() -> Option<MagnitudeTimelineShader> {
        let shader_program = ShaderProgram::new_from_string(
            include_str!("resources/magnitude_timeline.vert"),
            include_str!("resources/basic.frag"),
        )
        .expect(&format!(
            "Failed to create MagnitudeTimelineShader: {}, {}.",
            "resources/magnitude_timeline.vert", "resources/basic.frag"
        ));
